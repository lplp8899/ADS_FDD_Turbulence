...
function [inno,residual,error_x,Kk,Pk,Pzk,x_ob_filter,P_ob_filter,z_ob_filter]...
    =UKF_fault_free(mean_x0,P_x0,x_real,z_real,Qk,Rk,u,g,delta_t,namedas,w_c0,w_ci,w_m,dim_sys,k,Qd_est_ff)


%----------------start of fault-free UKF----------------------
...
% time update
% x_ob=model_sys(sigma_x,u(:,k),g(:,k),delta_t);
x_ob=model_sys(sigma_x,u(:,k),g(:,k),delta_t);
x_ob_mean=x_ob*w_m;
...
z_ob_filter=model_output(x_ob_filter);
